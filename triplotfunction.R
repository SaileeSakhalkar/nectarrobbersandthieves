...
#                  values increase the margins around the plot, negative values orange3uce 
#                  them.
...
#          This version is dev4.4 (10 january 2017)
{
  ...
  if(plot.spe) {
    arrows(0, 0, spe.sc[,ax1], spe.sc[,ax2], length=ahead, angle=aangle, col="orange3")
    if(label.spe)
      text(spe.sc[,ax1], spe.sc[,ax2], labels = rownames(spe.sc), col="orange3", pos=pos.spe, cex=cex.char1)
  } else {
    if(label.spe)
      text(spe.sc[,ax1], spe.sc[,ax2], labels = rownames(spe.sc), col="orange3", pos=NULL, cex=cex.char1)
  }
  ...
  if(!arrows.only) {
    ...
    if(quant.env.present & plot.env) {   # Print arrows and labels for quantitative var.
      arrows(0, 0, BP.sc[to.plot,ax1]*mult.arrow, BP.sc[to.plot,ax2]*mult.arrow, length=ahead, angle=aangle, col="lightseagreen")
      if(label.env)   # Print labels for the quantitative variables
        text(BP.sc[to.plot,ax1]*mult.arrow, BP.sc[to.plot,ax2]*mult.arrow, labels = rownames(BP.sc)[to.plot], col="lightseagreen", pos=pos.env, cex=cex.char2)
    } else {
      if(quant.env.present & !plot.env & label.env)   # Only print labels for quant. var.
        text(BP.sc[to.plot,ax1]*mult.arrow, BP.sc[to.plot,ax2]*mult.arrow, labels = rownames(BP.sc)[to.plot], col="lightseagreen", pos=NULL, cex=cex.char2)
    }
    #
    # 2. Centroids and labels of factor levels
    if(centroids.present & plot.centr) {   # Print symbols and labels for factor classes
      points(centroids[,ax1], centroids[,ax2], pch=19, cex=cex.point, col="lightseagreen")
      if(label.centr)
        text(centroids[,ax1], centroids[,ax2], labels = rownames(centroids), col="lightseagreen", pos=pos.centr, cex=cex.char2)
    } else {
      #
      if(centroids.present & !plot.centr & label.centr)   # Only print labels for classes
        text(centroids[,ax1], centroids[,ax2], labels = rownames(centroids), col="lightseagreen", pos=NULL, cex=cex.char2)
    }
  }
  
  # 3. All env. var.: plot arrows and labels for all var. in 'BP.sc', quant. and factors
  if(arrows.only) {
    arrows(0, 0, BP.sc[,ax1]*mult.arrow, BP.sc[,ax2]*mult.arrow, length=ahead, angle=aangle, col="lightseagreen")
    if(label.env)  # Print labels for the quantitative variables
      text(BP.sc[,ax1]*mult.arrow, BP.sc[,ax2]*mult.arrow, labels = rownames(BP.sc), col="lightseagreen", pos=pos.env, cex=cex.char2)
  }
  #
}}